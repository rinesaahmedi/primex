    ...
    <section className="hero-section relative isolate overflow-hidden px-6 py-24 md:py-32 lg:py-40">
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-12 lg:gap-16 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        ...
        <div className="space-y-10 text-left text-white">
          <span className={`hero-pill animate-lift-blur-subtle ${isVisible ? 'visible' : ''}`}>{t("hero.kicker")}</span>

          <div className={`animate-lift-blur-subtle ${isVisible ? 'visible' : ''}`} style={{ transitionDelay: '0.1s' }}>
          ...
          <p className={`max-w-xl text-lg md:text-xl text-slate-100/90 animate-lift-blur-subtle ${isVisible ? 'visible' : ''}`} style={{ transitionDelay: '0.2s' }}>
            {t("hero.body")}
          </p>
          ...
          <p className={`hero-stats-text animate-lift-blur-subtle ${isVisible ? 'visible' : ''}`} style={{ transitionDelay: '0.4s' }}>{statsText}</p>
        </div>

        {/* Right column */}
        <div className={`hero-right-panel bg-white/10 border border-white/20 text-slate-900 animate-lift-blur ${isVisible ? 'visible' : ''}`} style={{ transitionDelay: '0.2s' }}>
          ...
          <div className="hero-right-grid grid grid-cols-1 gap-4 md:grid-cols-2">
            {highlights.map((item, index) => (
              <div
                key={item.title}
                className={`hero-right-card animate-lift-blur-subtle ${isVisible ? 'visible' : ''}`}
