...
const CustomerSupportPage = () => {
  ...
  const servicePillars = [
    ...
    {
      key: "afterSales",
      icon: <Heart className="w-8 h-8 text-[#F43F5E] mb-4" />, // Pink/Red for care
      bg: "bg-gradient-to-br from-rose-50 to-pink-50 border-rose-100",
      dotColor: "bg-rose-400"
    },
    {
      key: "claims",
      icon: <ShieldCheck className="w-8 h-8 text-[#F59E0B] mb-4" />, // Amber for security/claims
      bg: "bg-gradient-to-br from-amber-50 to-orange-50 border-amber-100",
      dotColor: "bg-amber-400"
    },
    {
      key: "retention",
      icon: <Users className="w-8 h-8 text-[#8B5CF6] mb-4" />, // Purple for community/retention
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div ref={sectionRef} className={`mb-12 ${isVisible ? "lift-up-subtle" : ""}`}>
          ...
          <div className="flex flex-wrap gap-3 mb-8">
            {t("customerSupport.highlights", { returnObjects: true }).map((highlight, index) => (
              <span key={index} className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 rounded-full text-sm font-medium text-slate-700">
        ...
        {/* 2. Featured Image Section (The Photo) */}
        <div 
        ...
        >
          <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-slate-100 group">
            {/* Image overlay gradient for text readability if needed, or just aesthetics */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent pointer-events-none" />
            
            <img 
              src={supportHeroImage} 
              alt="PrimEx Customer Support Team Dashboard" 
            ...
            <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 bg-white/95 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-white/50 max-w-xs hidden md:block">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="font-bold text-gray-900">Optimized 24/7</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {servicePillars.map((pillar) => {
              const data = t(`customerSupport.services.${pillar.key}`, { returnObjects: true });
              
              return (
                <div key={pillar.key} className={`${pillar.bg} rounded-2xl p-8 border hover:shadow-lg transition-all duration-300`}>
                  {pillar.icon}
                  <h3 className="text-2xl font-bold text-gray-900 mb-3">{data.title}</h3>
                  <p className="text-gray-600 mb-6 min-h-[3rem]">{data.description}</p>
                    ...
                    <ul className="space-y-3">
                      {data.features && data.features.map((feature, idx) => (
                        <li key={idx} className="flex items-start gap-3 text-sm text-gray-700 font-medium">
                          <span className={`mt-1.5 w-2 h-2 rounded-full ${pillar.dotColor} shrink-0`} />
                          <span>{feature}</span>
                        </li>
                      ))}
                    </ul>
        ...
        >
          ...
          <div className="bg-white rounded-2xl p-8 border-2 border-slate-200">
            <ul className="grid md:grid-cols-2 gap-x-6 gap-y-4">
              {t("customerSupport.whyChoose.items", { returnObjects: true }).map((benefit, index) => (
